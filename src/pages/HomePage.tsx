...
type NavLink = { label: string; active?: boolean; dropdown?: boolean; to?: string }

const navLinks: NavLink[] = [
  { label: 'Home', active: true, to: '/' },
  { label: 'Shop', dropdown: true, to: '/shop' },
  { label: 'Gifts', to: '/shop' },
  { label: 'Our Story' },
]
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-neutral-900">
      ...
      <nav className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-5 sm:px-8 py-5">
        ...
        <button
          onClick={() => go('/')}
          className="flex items-center gap-2.5 text-white font-medium text-base"
        >
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button
            onClick={() => go('/shop')}
            className="bg-[#C53735] text-white text-sm font-medium px-4 py-2.5 rounded-full hover:bg-[#AE2F2D] transition-colors"
          >
            Shop now
          </button>
        </div>
      ...
      {menuOpen && (
        <div className="absolute top-[72px] left-4 right-4 z-30 md:hidden liquid-glass rounded-2xl p-4 flex flex-col gap-1">
          ...
          <div className="flex gap-2 mt-2 pt-3 border-t border-white/10">
            ...
            <button
              onClick={() => go('/shop')}
              className="flex-1 bg-[#C53735] text-white text-sm font-medium px-4 py-2.5 rounded-full hover:bg-[#AE2F2D] transition-colors"
            >
              Shop now
            </button>
          </div>
      ...
      <div className="absolute bottom-0 left-0 z-20 px-6 sm:px-12 pb-10 sm:pb-16 max-w-2xl">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            onClick={() => go('/shop')}
            className="bg-[#C53735] text-white text-sm sm:text-base font-medium px-6 sm:px-7 py-3 rounded-full hover:bg-[#AE2F2D] transition-colors"
          >
            Shop gifts
          </button>
